...
# CHANGELOG: 
#  2017-08-11 v0.804 Adam: Partially modernized for Python 3
...
#  2014-06-28 v0.705 Karsten: switching to optparse, allow ignoring mark codepoints, output list of missing mark
# codepoints
...
#  2014-06-25 v0.701 Karsten: test font, add ccmp to gsub, add Dummy DSIG, remove contours/components, kerning from
# gpos ppf1, win name records, win cmap subtables, post glyphnames
...
%s """ % (TOOL_URL)
    ...
    try:
        ...
        if skipMarksAddkToDefault:
            ...
            SKIP_MARKS_FINAL = skipMarks
    SKIP_MARKS_FINAL = SKIP_MARKS_FINAL.strip().replace(",", " ").replace(";", " ").replace("-", " ").replace("/",
                                                                                                              " "
                                                                                                              "").split()
    ...
    SKIP_MARKS_FINAL = [int(m, 16) for m in SKIP_MARKS_FINAL if len(m)];
    m = None
...
MARK_GLYPH_CODEPOINT_RANGE = [int(m) for m in range(65000) if unicodedata.category(unichr(m)) == "Mn"];
m = None  # also allow for "M"?
                    ...
                    if isMark:
                        if n in classdefs:
                            if classdefs[n] != 3 and CORRECT_MARK_CLASS:
                                classdefs[n] = 3
                                if VERBOSE: print(
                                    "'GDEF' table's GlyphClassDef doesn't flag glyph '%s' as mark. Corrected." % n)
                        else:
                            # do that anyway ...
                            classdefs[n] = 3
                            if VERBOSE: print(
                                "'GDEF' table's GlyphClassDef doesn't contain mark glyph '%s'. Added." % n)
            n = None
...
def renameFont(ttx):
    ...
    space = "".join([pack(">" + "H", ord(char)) for char in " "]);
    char = None
    renameWith = "".join(
        [pack(">" + "H", ord(char)) for char in RENAME_FONT_ADDITION.strip().strip("'").strip('"') + " "]);
    char = None
    renameWithout = "".join(
        [pack(">" + "H", ord(char)) for char in RENAME_FONT_ADDITION.strip().strip("'").strip('"')]);
    char = None
                ...
                if subtable.Format == 1:
                    ...
                    for gIdx in range(len(subtable.Coverage.glyphs) - 1, -1, -1):
                        if subtable.Coverage.glyphs[gIdx] in glyphs_removeOutlinesAndInstructions:
                            del subtable.Coverage.glyphs[gIdx]
                    # class defs:
                    class1Before = cleanUpList(
                        [int(subtable.ClassDef1.classDefs[g]) for g in subtable.ClassDef1.classDefs] + [
                            0])  # includes 0
                    class2Before = cleanUpList(
                        [int(subtable.ClassDef2.classDefs[g]) for g in subtable.ClassDef2.classDefs] + [
                            0])  # includes 0
                    ...
                    for g1 in glyphs1:
                        try:
                            class1glyphs[subtable.ClassDef1.classDefs[g1]] += [g1]
                        except:
                            class1glyphs[subtable.ClassDef1.classDefs[g1]] = [g1]
                    ...
                    for g2 in glyphs2:
                        try:
                            class2glyphs[subtable.ClassDef2.classDefs[g2]] += [g2]
                        except:
                            class2glyphs[subtable.ClassDef2.classDefs[g2]] = [g2]
                        if g2 in glyphs_removeOutlinesAndInstructions:
                            del subtable.ClassDef2.classDefs[g2]
                    class1After = cleanUpList(
                        [int(subtable.ClassDef1.classDefs[g]) for g in subtable.ClassDef1.classDefs] + [
                            0])  # includes 0
                    class2After = cleanUpList(
                        [int(subtable.ClassDef2.classDefs[g]) for g in subtable.ClassDef2.classDefs] + [
                            0])  # includes 0
            ...
            for fiIdx in range(len(scriptRecord.Script.DefaultLangSys.FeatureIndex) - 1, -1, -1):
                if scriptRecord.Script.DefaultLangSys.FeatureIndex[fiIdx] >= ccmpIdx:
                    scriptRecord.Script.DefaultLangSys.FeatureIndex[
                        fiIdx] += 1  # we added a feature, so need to increase the reference-by-index of all that follow
            scriptRecord.Script.DefaultLangSys.FeatureIndex += [ccmpIdx]
            scriptRecord.Script.DefaultLangSys.FeatureIndex.sort()  # are feature indices expected to be sorted in
            # ascending order?
                    ...
                    for fiIdx in range(len(langSysRecord.LangSys.FeatureIndex) - 1, -1, -1):
                        if langSysRecord.LangSys.FeatureIndex[fiIdx] >= ccmpIdx:
                            langSysRecord.LangSys.FeatureIndex[
                                fiIdx] += 1  # we added a feature, so need to increase the reference-by-index of all
                            # that follow
                    langSysRecord.LangSys.FeatureIndex += [ccmpIdx]
                    langSysRecord.LangSys.FeatureIndex.sort()  # are feature indices expected to be sorted in
                    # ascending order?
...
def main(inPath, outPath):
        ...
        try:
            ...
            if len(dec) > 1:
                if not dec[:1] == "<":
                    ...
                    decall = 0
                    for ud in udec:
                        if ud in SKIP_MARKS_FINAL:  # if mark is in SKIP_MARKS_FINAL we don't want to do any
                            # decomposition
                            return 0
                        if ud in umap:
                            decall += 1
                    ...
                    if decall == len(
                            udec) and decall > 1:  # the last condition may go for the sake of allowing reference to
                        # same-shape glyphs
    ...
    if umap:
        for u in umap:
            ...
            while decomp:
                ...
                # cf: 01E0;LATIN CAPITAL LETTER A WITH DOT ABOVE AND MACRON;Lu;0;L;0226 0304;;;;N;LATIN CAPITAL
                # LETTER A DOT MACRON;;;01E1;
                if decomp:
    ...
    if VERBOSE: print("Diet efficiency: %s%% (from %s to %s bytes)" % (
    float(int((1 - float(outSize) / inSize) * 10000)) / 100, inSize, outSize))
